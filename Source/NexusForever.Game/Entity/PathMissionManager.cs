...
{
    public class PathMissionManager
    {
        ...
        private void SendServerPathEpisodeProgress(uint episodeId, IEnumerable<PathMission> pathMissions)
        {
            ...
            foreach(PathMission pathMission in pathMissions)
            {
                ...
                {
                    PathMissionId             = (ushort)pathMission.Id,
                    Completed                 = pathMission.IsComplete(),
                    ObjectiveCompletionFlags  = pathMission.Progress,
                    StateFlags                = (uint)(pathMission.Progress > 0 && !pathMission.IsComplete() ? 1 : 0)
                });
            }
        ...
        private void SendServerPathMissionActivate(IEnumerable<PathMission> pathMissions, byte reason = 1, uint giver = 0)
        {
            ...
            foreach (PathMission pathMission in pathMissions)
            {
                ...
                missionList.Add(new ServerPathMissionActivate.Mission
                {
                    ...
                    Completed                = pathMission.IsComplete(),
                    ObjectiveCompletionFlags = pathMission.Progress,
                    StateFlags               = (uint)(pathMission.IsComplete() ? 0 : 1),
                    State                    = pathMission.IsComplete()
                        ? ServerPathMissionActivate.Mission.PathMissionState.Complete
                        : ServerPathMissionActivate.Mission.PathMissionState.Unlocked,
                    GiverUnitId              = giver
                });
            }
